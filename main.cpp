...
private:
    ...
    Aspect compute_left_side_bearing(const std::wstring& text) const noexcept {
        ...
        for(auto c: text) {
            ...
            lsb = w - lsb;
            if(a.max_lsb > lsb) a.max_lsb = lsb;
            w += advance;
            if(a.max_width < w) a.max_width = w;
...
public:
    ...
    std::vector<unsigned char> bake(const std::wstring& text, const float resolution, int &img_width, int &img_height) const noexcept {
        const float fnt_scale = resolution / float(fnt_height);
        std::cout << "scale: " << fnt_scale << std::endl;
        const int base_line = ascent * fnt_scale;
        std::cout << "base_line: " << base_line << std::endl;
        std::vector<unsigned char> img_pixels;
        compute_left_side_bearing(text).print();
        return img_pixels;    
    }
};
...
int main(int arg, char **argv)
{
    ...
    const auto img_data = fnt.bake(L"Hello World", 15.0f, img_width, img_height);
//     for(int i = 0 ; i < 100; ++i)
//     for(int j = 0 ; j < 20; ++j)
//     screen[j][i] = 0;
